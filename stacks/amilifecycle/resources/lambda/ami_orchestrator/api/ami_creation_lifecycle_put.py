...
"""
    ami_creation_lifecycle_put.py: 
    lambda handler for the ami creation put url:
    PUT: https://{api_endpoint}/ami-creation/lifecycles
...
def lambda_handler(event, context):
    ...
    try:
        ...
        security_service.is_ami_creation_put_authorized(definition['api_key'])

        # validate the structure and basic logic of request 
        # if the request is not valid a ValidationError exception will be raised
        validator_service.validate_lifecycle_definition_create_update_request(definition)

        # write the job definition to dynamodb
        lifecycle_defintion = database_service.create_update_lifecycle(definition)
        ...
        response = {
            ...
            "creation_date": lifecycle_defintion['update_date'],
