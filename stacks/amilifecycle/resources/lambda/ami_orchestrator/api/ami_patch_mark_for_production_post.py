...
"""
    ami_patch_mark_for_production_post.py: 
    lambda handler for the ami patch mark for production url:
    POST: https://{api_endpoint}/ami-patch/lifecycles/{lifecycle-id}/approve
...
OPERATOR = "AMI_PATCH_MARK_FOR_PRODUCTION_APPROVAL_POST"
...
def lambda_handler(event, context):
    ...
    try:
        ...
        security_service.is_ami_patch_mark_for_production_authorized(definition['api_key'])
        ...
        if 'outputs_ami_patch' in lifecycle_definition:
            if 'events' in lifecycle_definition['outputs_ami_patch']:
                for event in lifecycle_definition['outputs_ami_patch']['events']:
                    if event['name'] == constants_service.EVENT_MARK_FOR_PRODUCTION_PATCH:
        ...
        rules_service.validate_event_can_proceed_patch(
            current_event=constants_service.EVENT_MARK_FOR_PRODUCTION_PATCH,
        ...
        if definition['approval_status'] == "APPROVED":
            ...
            # grab ami details
            ami_detail = ami_details_service.get_ami_details_for_testing_patch(lifecycle_definition)    

            state_machine_input = {
                "lifecycle_id": lifecycle_definition['lifecycle_id'],
                "operation_type": constants_service.AMI_PATCH,
            ...
            approval_event = {}
            approval_event['name'] = constants_service.EVENT_MARK_FOR_PRODUCTION_PATCH
