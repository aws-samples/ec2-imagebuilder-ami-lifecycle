...
"""
    ...
    AMI Lifecycle AMI_PATCH State Machine.
"""
...
# constants
OPERATOR = "AMI_PATCH_GET_AMI_DETAILS"
...
def lambda_handler(event, context):
    ...
    try:
        ...
        event['patch_ami_operation']['output']['ami_state'] = str(ami_state).upper()
        event['patch_ami_operation']['output']['image'] = response['image']
        event['patch_ami_operation']['output']['ami_details'] = response['image']['outputResources']['amis']

        # remove some keys we do not need
        del event['patch_ami_operation']['output']['image']['infrastructureConfiguration']
        del event['patch_ami_operation']['output']['image']['distributionConfiguration']
        del event['patch_ami_operation']['output']['image']['imageTestsConfiguration']
        del event['patch_ami_operation']['output']['image']['imageRecipe']['blockDeviceMappings']
